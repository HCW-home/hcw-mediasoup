...
class Room extends EventEmitter
{
	...
	static getLeastLoadedRouter(mediasoupWorkers, peers, mediasoupRouters)
	{
		...
		for (const [ routerId, router ] of mediasoupRouters)
		{
			for (const worker of mediasoupWorkers)
			{
				if (routerId && worker._pid)
				{
			...
			pipedRoutersIds.size === mediasoupRouters.size)
		{
			...
			for (const worker of mediasoupWorkers)
			{
				...
				{
					if (!worker._routers || !worker._routers.size)
						continue;
		...
		if (mediasoupRouters.size > 0)
		{
			return mediasoupRouters.keys().next().value;
		}
	...
	{
		console.log('Peer joining', peer.id, returning)
